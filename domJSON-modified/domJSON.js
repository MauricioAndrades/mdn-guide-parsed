...
(function (root, factory) {
	...
	var ignored = ["childNodes", "children", "classList", "dataset", "style"];
	var serials = ["innerText", "prefix", "textContent", "text"];
	...
	var copyJSON = function (node, opts) {
		...
		_copy = boolFilter(_copy, opts.domProperties);
		return _copy;
	};
	...
	var toJSON = function (node, opts, depth) {
		var style, child_nodes, child_nodes_len, thisChild;
		var children = [];
		node.normalize();
		...
		if (node.nodeType === 1) {
			for (var b in banned) {
				if (node.tagName.toLowerCase() === banned[b]) {
					return JSON.stringify(node.toString().trim());
				}
			}
		...
		if ( opts.attributes && node.hasAttributes() ) {
			try {
		...
		if (opts.deep === true || typeof opts.deep === "number" && opts.deep > depth) {
			if (node.hasChildNodes && node.children) {
				...
				child_nodes_len = node.children.length;
				if (child_nodes) {
					for (var c = 0; c < child_nodes_len; c++) {
						if (child_nodes[c] && child_nodes[c] !== null && child_nodes[c] !== undefined) {
							try {
								children.push(toJSON(child_nodes[c], opts, depth + 1));
							} catch (e) {
								console.log(e);
							}
						}
...
var json = window.domJSON.toJSON(qs_node('body'), opts);
